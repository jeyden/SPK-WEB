    ...
    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="flex items-center gap-4 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
            <div class="relative shrink-0">
                @php
                    $currentAvatar = null;
                    if ($user->role === 'student' && $user->student && $user->student->photo) {
                        $currentAvatar = asset('storage/' . $user->student->photo);
                    } elseif ($user->avatar) {
                        $currentAvatar = asset('storage/' . $user->avatar);
                    }
                @endphp
        ...
        <div>
            <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">NIP (Nomor Induk Pegawai)</label>
            <div class="relative">
                <span class="absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400 dark:text-slate-500">
