...
<div class="max-w-2xl mx-auto space-y-6 pb-16">
    ...
    <section class="rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 px-5 py-5 sm:px-6">
        ...
        <h1 class="mt-2 text-lg font-black tracking-tight text-slate-900 dark:text-white sm:text-xl">Buat Periode Baru</h1>
        <p class="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
            Periode baru akan berstatus "Belum Dibuka" dan dapat dibuka kapan saja dari halaman daftar.
        </p>
    </section>
    ...
    <div class="bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm px-5 py-6 sm:px-8">
        <form action="{{ route('counselor.registration-periods.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="academic_year"
                    ...
                    <option value="" disabled {{ old('academic_year') ? '' : 'selected' }}>Pilih tahun akademik</option>
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Catatan tambahan mengenai periode ini"
                          class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">{{ old('description') }}</textarea>
