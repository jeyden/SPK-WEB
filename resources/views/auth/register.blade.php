...
<head>
    ...
    <title>Register - Sistem Pendukung Keputusan Pemilihan Jurusan</title>
    <!-- FAVICON KUSTOM -->
        ...
        <div class="glass border border-white/10 rounded-3xl shadow-2xl shadow-indigo-950/40 overflow-hidden">
            <div class="p-6 sm:p-8">
                ...
                <div class="text-center mb-6">
                    ...
                    <h2 class="text-2xl font-bold text-white tracking-tight mt-1">Buat Akun Baru</h2>
                    <p class="text-xs text-slate-400 mt-1">Lengkapi data di bawah ini untuk mendaftar ke sistem</p>
                </div>
                ...
                @if ($errors->any())
                    <div class="mb-4 p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-200 text-xs flex items-center gap-2">
                        ...
                        <span>Periksa kembali input data Anda di bawah ini.</span>
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="Nama Lengkap Anda"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required autocomplete="email" placeholder="nama@sekolah.sch.id"
                                class="w-full pl-11 pr-4 py-2.5 bg-white/[.04] border border-white/10 rounded-xl text-white placeholder-slate-600 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition">
                        </div>
                        @error('email')
                            <span class="text-rose-400 text-[11px] mt-1 block">{{ $message }}</span>
                ...
                <p class="text-center text-xs text-slate-400">
                    Sudah punya akun? 
                    <a href="{{ route('login') }}" class="text-indigo-400 hover:text-indigo-300 font-semibold ml-1 transition">Masuk di sini</a>
                </p>
            </div>
        </div>
